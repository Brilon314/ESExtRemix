function comercio() {
	...
	if (LOCAL.getImperio().raza == null) {
		alert(
			"Raza no identificada, por favor ve a 'Tu Imperio' para actualizar la información.",
		);
		return;
	}
	...
	$(".lista1 tr").each(function (index, obj) {
		...
		if (difCompra == 0)
			$(obj.children[2]).append(
				"<img src='https://images.empire-strike.com/v2/iconos/icon_correcto.png' alt='Precio óptimo' title='Precio óptimo'>",
			);
		else
			$(obj.children[2]).append(
				"<span style='font-size:11px'><b style='color:#990000'>+" +
					difCompra +
					"</b></span>",
			);
		...
		$(obj.children[2]).append(
			`<span style='font-size:11px'> = <b id='bc${nombre}'></b></span>`,
		);

		$(obj.children[3]).append(
			`<span style='font-size:11px'> = <b id='bv${nombre}'></b></span>`,
		);

		document.querySelector("#" + nombre.toLowerCase()).onkeyup =
			function () {
				compraJusta(oroRaza);
			};
		...
		document.querySelector("#c" + nombre).onclick = function () {
			compraJusta(oroRaza);
		};
		...
		document.querySelector("#v" + nombre).onclick = function () {
			compraJusta(oroRaza);
		};
	});
	document.querySelector("#btn_venderTodo").onclick = function () {
		compraJusta(oroRaza);
	};
	var selectRecurso = "";
	var selectVenta = 0;
...
function compraJusta(oroRaza) {
	...
	var oroGanado = parseInt(
		document.getElementById("cobrar").value.replaceAll(".", ""),
	);
	var oroGastado = parseInt(
		document.getElementById("pagar").value.replaceAll(".", ""),
	);
