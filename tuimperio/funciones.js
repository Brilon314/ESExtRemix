...
function calculaBelico(indiceBelico){
	var ibReducidoAlPaso = 0.1*(100-indiceBelico);
	var ibAlPaso = (indiceBelico - ibReducidoAlPaso).toFixed(1);
	if(ibAlPaso < 0)
		ibAlPaso = 0;

	var idReducido = indiceBelico;
	var count = 0;
	while(15 <= idReducido){
		count++;
...
function mostrarBelico(indiceBelico,ibAlPaso,count){
	if(isPacifico(indiceBelico)){
		...
		$("#datos>tbody>tr:eq(4)").append(`<td><b>IB al paso:</b> ${ibAlPaso}%, necesitas ${count} paso(s) para volver a pacífico</td>`);
